...
/* immidiate枠には次の6種類がある。
          imm[11:0]  (基本はこれ),
          shamt[4:0] (shift系),
          imm[31:12] (lui,aupci), imm[19:0]  (jal),
          imm[31:0]  (li)
   次の関数では、bit = 11, 4, 19, 31 で場合分けしています。1048576 は 1<<20 の値。*/
...
signed char instruction(char t[256]){
  if(mode==1) return -1;
    ...
    if(im!=1048576){
      ...
      // beqで飛べる場合。
      if(-2048<=distance&&distance<2048){
        struct instruction type_B;
        type_B.opcode = 0b1100011;
        type_B.funct3 = 0b000;
        type_B.rs1_index = index_register(rs1);
      ...
      // 飛べない場合。
        ...
        store_text(i,type_U);
        text_address = text_address+1;
        distance = distance-(dis_lui<<12);
        struct instruction type_I;
        type_I.opcode = 0b0010011;
        type_I.funct3 = 0b000;
        type_I.rd_index = index_register("t1");
        type_I.rs1_index = index_register("t1");
        type_I.imm = distance>>1;
        ...
        type_B.rs1_index = index_register(rs1);
        type_B.rs2_index = index_register(rs2);
        type_B.imm = distance-((distance>>1)*2);
        i = index_text(text_address);
        if(i<0||type_B.rs1_index<0||type_B.rs2_index<0) return -1;
        store_text(i,type_B);
        text_address = text_address+1;
    ...
    // ラベルの場合
      ...
      int32_t distance = address - text_address;
      // jalで飛べる場合。
      if(-524288<=distance&&distance<524288){
        struct instruction type_J;
        type_J.opcode = 0b1101111;
